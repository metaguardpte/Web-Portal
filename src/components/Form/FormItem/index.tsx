...
const FormItem = (
    props: FormItemProps & {
        ...
        extraStyle?: React.HTMLAttributes<HTMLDivElement>['style'];
    },
) => {
    ...
    if (rules) {
        ruleData = rules.map((rule) => {
            ...
            const validator = (_: any, value: any) => {
                return new Schema(config).validate({ test: value }, (error) => {
            ...
            <Form.Item {...rest} validateFirst rules={ruleData} noStyle>
                {React.isValidElement(children) ? (
                    React.cloneElement(children, {
                        innerStyle: { minHeight: props.extraStyle?.minHeight },
                        ...children.props,
                        wrapperStyle: {
                            borderRadius: props.extraStyle?.borderRadius,
                            ...children.props.wrapperStyle,
                            ...wrapperStyle,
                        },
                        label: props.label,
                    })
